...
import { getEmployeeById as getEmployeeByIdAction, updateEmployee as updateEmployeeAction, deleteEmployeeAction } from '@/modules/employees/actions';
...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  try {
    const employee = await getEmployeeByIdAction(params.id, tenantId); // Call the server action with tenantId
    if (!employee) {
...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    // Add tenantId for validation and action call
    ...
    // Call the server action to update the employee (action should handle tenantId)
    // Pass tenantId explicitly to action for verification
    const result = await updateEmployeeAction(params.id, tenantId, validation.data);
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  try {
    // Call the server action to delete the employee (pass tenantId for verification)
    const result = await deleteEmployeeAction(params.id, tenantId);
    if (result.success) {
